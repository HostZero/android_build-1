...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^zeus_") ; then
       ZEUS_BUILD=$(echo -n $1 | sed -e 's/^zeus_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $ZEUS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       ZEUS_BUILD=
    fi
    export ZEUS_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    ZEUS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/zeus/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the ZEUS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch zeus_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the ZEUS github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var ZEUS_VERSION)
        ZIPFILE=zeus-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.zeus.device | grep -q "$ZEUS_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $ZEUS_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $ZEUS_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/zeus/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then

...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from ZEUS Github (http://github.com/Zeus-devices)." % device)

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/Zeus-devices/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/zeus_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    f = open('.repo/local_manifests/zeus_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for ZEUS/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print('Adding dependency: ZEUS/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "ZEUS/%s" % repo_name })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/zeus.dependencies'
    syncable_repos = []
    ...
    if os.path.exists(dependencies_path):
        ...
        for dependency in dependencies:
            if not is_in_manifest("ZEUS/%s" % dependency['repository'], "%s" % dependency['branch']):
...
print("Repository for %s not found in the ZEUS Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/zeus_manifest.xml." % device)
